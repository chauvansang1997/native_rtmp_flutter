...
class CameraNativeView(
    ...
    fun startPreview(cameraNameArg: String? = null) {
        ...
        val previewSize = CameraUtils.computeBestPreviewSize(cameraName, Camera.ResolutionPreset.ultraHigh)
        ...
        if (isSurfaceCreated) {
            try {
                ...
                rtmpCamera.startPreview(if (isFrontFacing(targetCamera)) FRONT else BACK, sizes[0].width, sizes[0].height)
            } catch (e: CameraAccessException) {
//                close()
